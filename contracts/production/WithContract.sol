...
contract WithContractProduction is VRFV2WrapperConsumerBase{
    ...
    function getRandomWord() view internal returns (uint256) {
        return statues[requestId].randomWord;
    }


    function calculateResults(uint256 _playerItem, uint256 contractItem ) internal view returns (uint256){
    ...
    function playWithContract(uint256 _playerItem, uint256 betAmount) public {
        ...
        link.transferFrom(msg.sender, address(this), betAmount * 10 ** 18);
        requestRandomNumber();  
        ...
        if(result == 11){
            uint256 takeDrawCut = betAmount.mul(75 * 10 ** 16);
            link.transfer(msg.sender, takeDrawCut);
        }
        if(result == 1){
            uint256 winAmount = betAmount.mul(175 * 10 ** 16);
